from ariadne import MutationType

from app.db.security import create_access_token
from app.services.auth_service import login_service, register_service
from app.services.project_service import (
    create_project_service,
    delete_project_service,
    update_project_service,
)
from app.services.task_service import (
    change_task_status_service,
    create_task_service,
    delete_task_service,
    update_task_service,
)

mutation = MutationType()


@mutation.field("register")
def resolve_register(_, info, input):
    db = info.context["db"]
    user = register_service(db, input["name"], input["email"], input["age"], input["password"])
    token = create_access_token({"sub": str(user.id), "role": user.role})
    return {"accessToken": token, "tokenType": "bearer"}


@mutation.field("login")
def resolve_login(_, info, input):
    db = info.context["db"]
    result = login_service(db, input["email"], input["password"])
    return {"accessToken": result["access_token"], "tokenType": result["token_type"]}


@mutation.field("createProject")
async def resolve_create_project(_, info, input):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    return await create_project_service(db, input["name"], user.id, redis_pool)


@mutation.field("updateProject")
async def resolve_update_project(_, info, id, input):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    is_admin = user.role == "admin"
    return await update_project_service(db, id, input.get("name"), user.id, redis_pool, is_admin)


@mutation.field("deleteProject")
async def resolve_delete_project(_, info, id):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    is_admin = user.role == "admin"
    return await delete_project_service(db, id, user.id, redis_pool, is_admin)


@mutation.field("createTask")
async def resolve_create_task(_, info, input):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    return await create_task_service(db, input["title"], input["projectId"], user.id, redis_pool)


@mutation.field("updateTask")
async def resolve_update_task(_, info, id, input):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    is_admin = user.role == "admin"
    return await update_task_service(
        db,
        id,
        input.get("title"),
        input.get("projectId"),
        input.get("done"),
        user.id,
        redis_pool,
        is_admin,
    )


@mutation.field("deleteTask")
async def resolve_delete_task(_, info, id):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    is_admin = user.role == "admin"
    return await delete_task_service(db, id, user.id, redis_pool, is_admin)


@mutation.field("changeTaskStatus")
async def resolve_change_task_status(_, info, id, done):
    user = info.context["user"]
    db = info.context["db"]
    redis_pool = info.context["redis"]
    if not user:
        raise Exception("Not authenticated")
    is_admin = user.role == "admin"
    return await change_task_status_service(db, id, done, user.id, redis_pool, is_admin)
