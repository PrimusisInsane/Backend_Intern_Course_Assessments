...
async def create_project_service(db, name: str, user_id: int, redis_pool):
    ...
    await redis_pool.enqueue_job(
        "write_activity_log", user_id=user_id, action="created",
        project_id=project.id, detail=f"Project '{name}' created"
    )
...
async def delete_project_service(db, project_id, user_id: int, redis_pool, is_admin: bool = False):
    ...
    await redis_pool.enqueue_job(
        ...
        project_id=project_id, detail=f"Project '{name}' deleted"
    )
...
async def update_project_service(db, project_id, name: str, user_id: int, redis_pool, is_admin: bool = False):
    project = get_project_by_id(db, project_id)
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    if not is_admin:
    ...
    await redis_pool.enqueue_job(
        ...
        project_id=project_id, detail=f"Name changed to '{name}'"
    )
